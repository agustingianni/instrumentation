...
class TraceReader:
    ...
    def parse(self, match_events=""):
        ...
        info("Parsing trace file %s of size %f MB" % (self.filename, float(self.file_size) / 1024.0 / 1024.0))
                
        for line in self.f:
            if bytes_processed > self.size_limit and self.size_limit != 0:
                info("Reached trace file size limit of %d mega-bytes" % (self.size_limit / 1024 / 1024))
                break
            ...
            if (i % 500000) == 0:
                ...
                info("Processed %3d percent of trace file at %3d mb/s" % (bytes_processed * 100 / self.file_size, throughput))
                cur_time = time()
                current_bytes = 0
        ...
        info("Processed %3d percent of trace file at %3d mb/s" % (100, throughput))
        info("Trace loading took %d seconds" % (time() - start_time))
                
        info("Building 'write to chunk' index")
        chunk_cache = {}
        ...
        info("Finished building indexes")
